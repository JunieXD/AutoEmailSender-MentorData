...
软件应支持多个只读镜像基地址，但所有镜像必须提供相同的 Manifest 与哈希。客户端不得接受生成时间早于本地已验证缓存的版本；生成时间相同但数据版本不同也必须视为镜像冲突。
...
已有本地导师第一次匹配时使用全部当前规范化邮箱和冲突保护；建立关联后始终以稳定 `community_record_id` 跟踪，即使主邮箱变化也不会创建新导师。
...
社区记录完整保留多个联系方式和多个当前任职。软件默认写入唯一主要当前邮箱与唯一主要当前任职；其他联系方式和任职在预览页展示，不丢失社区侧结构。`retired`、`departed`、`deceased`、`disputed`、`removed` 等状态通过撤销列表同步到关联表，只生成提醒，不删除、归档或恢复本地导师。超过 `publication.stale_after_days` 未重新核验的活跃记录会从导入分片移入 `stale` 撤销记录。活跃导师发生主任职调动时，发布器在撤销文件的 `events` 中追加向后兼容的 `kind: relocation` 事件。为兼容严格校验 v2 事件字段的旧客户端，事件的 `observed_at` 取官网证据时间与可信审核完成时间中较晚者，表示事件开始对客户端可见的时间；规范导师记录仍保留原始证据时间。客户端按事件 `observed_at` 提醒旧导入用户，但保持关联为 `active`，不把调动误判为退休。
