...
建议每次提交同一所学校、同一个学院的数据，审核会更快；确有需要时，一个共享包也可以包含多个学校和学院。学校和学院名称请尽量使用官网全称，简称也可以提交并由维护者统一。每个文件最大 5 MiB、最多 5,000 行。
...
- 每个 Issue 只使用一条固定的审核记录；审核通过后会自动按顺序落库并发布，发布成功后 Issue 自动关闭。
...
只提交高校、学院、系所、研究院或实验室官网已经公开的职业信息：姓名、公开工作邮箱、职称、任职机构、研究方向、少量代表论文标题、高校官网导师详情页和发现导师的列表/目录来源页。
